...
bool ChatHandler::HandleLinkAddCommand(char* args)
{
	...
	if (QueryResult* result = WorldDatabase.PQuery("SELECT flag FROM creature_groups WHERE leaderGUID = '%u' AND master_guid = '%u'", player->GetSelectionGuid().GetCounter(), masterCounter))
	{
		Field* fields = result->Fetch();
	...
	{
		WorldDatabase.PExecute("INSERT INTO creature_groups(memberGUID,leaderGUID,flag) VALUES('%u','%u','%u')", player->GetSelectionGuid().GetCounter(), masterCounter, flags);
		PSendSysMessage("Created link for memberGUID = %u , leaderGUID = %u and flags = %u", player->GetSelectionGuid().GetCounter(), masterCounter, flags);
	}
...
bool ChatHandler::HandleLinkRemoveCommand(char* args)
{
	...
	if (QueryResult* result = WorldDatabase.PQuery("SELECT flag FROM creature_groups WHERE memberGUID = '%u' AND leaderGUID = '%u'", player->GetSelectionGuid().GetCounter(), masterCounter))
	{
		delete result;
		WorldDatabase.PExecute("DELETE FROM creature_groups WHERE memberGUID = '%u' AND leaderGUID = '%u'", player->GetSelectionGuid().GetCounter(), masterCounter);
		PSendSysMessage("Deleted link for memberGUID = %u and master_guid = %u", player->GetSelectionGuid().GetCounter(), masterCounter);
	}
	else
...
bool ChatHandler::HandleLinkEditCommand(char* args)
{
	...
	if (QueryResult* result = WorldDatabase.PQuery("SELECT flag FROM creature_groups WHERE memberGUID = '%u' AND leaderGUID = '%u'", player->GetSelectionGuid().GetCounter(), masterCounter))
	{
		...
		if (flags)
		{
			WorldDatabase.PExecute("UPDATE creature_groups SET flags = flags | '%u' WHERE memberGUID = '%u' AND leaderGUID = '%u'", flags, player->GetSelectionGuid().GetCounter(), masterCounter);
	...
	else
	{
		if (flags)
		{
			WorldDatabase.PExecute("INSERT INTO creature_groups(memberGUID,leaderGUID,flags) VALUES('%u','%u','%u')", player->GetSelectionGuid().GetCounter(), masterCounter, flags);
...
bool ChatHandler::HandleLinkToggleCommand(char* args)
{
	...
	if (QueryResult* result = WorldDatabase.PQuery("SELECT flag FROM creature_groups WHERE leaderGUID = '%u' AND master_guid = '%u'", player->GetSelectionGuid().GetCounter(), masterCounter))
	{
		...
		if (toggle)
		{
			WorldDatabase.PExecute("UPDATE creature_groups SET flags = flags &~ '%u' WHERE leaderGUID = '%u' AND master_guid = '%u'", flags, player->GetSelectionGuid().GetCounter(), masterCounter);
			SendSysMessage("Flag removed from link.");
		}
		else
		{
			WorldDatabase.PExecute("UPDATEcreature_groups SET flags = flags | '%u' WHERE leaderGUID = '%u' AND master_guid = '%u'", flags, player->GetSelectionGuid().GetCounter(), masterCounter);
			SendSysMessage("Flag added to link.");
		}
	}
	else
...
bool ChatHandler::HandleLinkCheckCommand(char* args)
{
	...
	if (QueryResult* result = WorldDatabase.PQuery("SELECT flag FROM creature_groups WHERE memberGUID = '%u' AND leaderGUID = '%u'", player->GetSelectionGuid().GetCounter(), masterCounter))
	{
		...
		PSendSysMessage("Link for memberGUID = %u , master_guid = %u has flags = %u", player->GetSelectionGuid().GetCounter(), masterCounter, flags);
		delete result;
		found = true;
	}

	if (QueryResult* result = WorldDatabase.PQuery("SELECT flag FROM creature_groups WHERE memberGUID = '%u' AND leaderGUID = '%u'", masterCounter, player->GetSelectionGuid().GetCounter()))
	{
		Field* fields = result->Fetch();
		uint32 flags = fields[0].GetUInt32();
		PSendSysMessage("Link for memberGUID = %u , leaderGUID = %u has flags = %u", masterCounter, player->GetSelectionGuid().GetCounter(), flags);
		delete result;
		found = true;
	}

	if (!found)
		PSendSysMessage("Link for memberGUID = %u , %u not found", masterCounter, player->GetSelectionGuid().GetCounter());
