...
void ElunaInstanceAI::Load(const char* data)
{
    ...
    if (decodedData)
    {
        ...
        if (lua_pcall(L, 1, 1, 0) == 0)
        {
            ...
            {
				sLog.outError("Error while loading instance data: Expected data to be a table (type 5), got type %d instead", lua_type(L, -1));
                lua_pop(L, 1);
                // Stack: (empty)
        ...
        {
            // Stack: error_message
			sLog.outError("Error while parsing instance data with lua-marshal: %s", lua_tostring(L, -1));
            lua_pop(L, 1);
            // Stack: (empty)
    ...
    {
		sLog.outError("Error while decoding instance data: Data is not valid base-64");
        Initialize();
    }
}
