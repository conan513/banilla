...
void Eluna::RunScripts()
{
    ...
    for (ScriptList::const_iterator it = scripts.begin(); it != scripts.end(); ++it)
    {
        ...
        {
			sLog.outError("[Eluna]: Error loading `%s`. File with same name already loaded from `%s`, rename either file", it->filepath.c_str(), loaded[it->filename].c_str());
            continue;
        }
        ...
        {
            lua_pop(L, 1);
			sLog.outError("[Eluna]: `%s` was already loaded or required", it->filepath.c_str());
            continue;
        }
        lua_pop(L, 1);
        // Stack: package, modules
        ...
        if (luaL_loadfile(L, it->filepath.c_str()))
        {
			...
			sLog.outError("[Eluna]: Error loading `%s`", it->filepath.c_str());
            Report(L);
            // Stack: package, modules
            continue;
        }
        ...
        if (ExecuteCall(0, 1))
        {
			...
			sLog.outError("[Eluna]: Successfully loaded `%s`", it->filepath.c_str());
            ++count;
            continue;
        }
    }
...
bool Eluna::ExecuteCall(int params, int res)
{
    ...
    {
		sLog.outError("[Eluna]: Cannot execute call: registered value is %s, not a function.", luaL_tolstring(L, base, NULL));
