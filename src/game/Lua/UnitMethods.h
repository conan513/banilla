    ...
    */
...
#else
        ChatHandler::FillMessageData(&data, target->GetSession(), ChatMsg(type), Language(lang), target->GET_GUID(), msg.c_str());
#endif
#ifdef CMANGOS
    ...
    int DealDamage(lua_State* L, Unit* unit)
    {
...
#else
            unit->SendAttackStateUpdate(HITINFO_NORMALSWING2, target, HITINFO_NORMALSWING2, SPELL_SCHOOL_MASK_NORMAL, damage, 0, 0, VICTIMSTATE_NORMAL, 0);
#endif
            return 0;
        }
        ...
        if (!spell)
        {
            ...
            unit->DealDamageMods(target, damage, &absorb);
            unit->DealDamage(target, damage, NULL, DIRECT_DAMAGE, schoolmask, NULL, false);
            unit->SendAttackStateUpdate(HITINFO_NORMALSWING2, target, HITINFO_NORMALSWING2, schoolmask, damage, absorb, resist, VICTIMSTATE_NORMAL, 0);
            return 0;
        }
