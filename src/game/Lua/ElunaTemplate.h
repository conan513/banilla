...
public:
    static int thunk(lua_State* L)
    {
        ...
        {
			sLog.outError("[Eluna]: %s returned unexpected amount of arguments %i out of %i. Report to devs", l->name, args, expected);
            ASSERT(false);
        }
...
public:
    ...
    static int Push(lua_State* L, T const* obj)
    {
        ...
        if (!ptrHold)
        {
			sLog.outError("%s could not create new userdata", tname);
            lua_pop(L, 2);
            lua_pushnil(L);
            return 1;
        }
        *ptrHold = new ElunaObject(const_cast<T*>(obj), manageMemory);
        ...
        if (!lua_istable(L, -1))
        {
			sLog.outError("%s missing metatable", tname);
            lua_pop(L, 3);
            lua_pushnil(L);
            return 1;
        }
