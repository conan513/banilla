...
struct npc_vimgol_AI : public ScriptedAI
{
	...
	void MovementInform(uint32 uiMovementType, uint32 uiData) override
	{
		...
		m_creature->CastSpell(m_creature, SPELL_UNHOLY_GROWTH, false);
...
struct npc_vimgol_middle_bunnyAI : public ScriptedAI
{
			...
			for (EntryGuidSet::iterator it = m_uiBunnyGuids.begin(); it != m_uiBunnyGuids.end(); ++it)
				if (it->second == bunny->GetObjectGuid())
					if (m_uiActiveCircles[std::distance(m_uiBunnyGuids.begin(), it)])
						bunny->CastSpell(pTarget ? pTarget : bunny, uSpell, true);
	}
	...
	void UpdateAI(const uint32 uiDiff) override
	{
		if (m_uiScanTimer <= uiDiff)
		{
			...
			if (m_uiSpawned)
			{
				if (Creature* pVimgol = GetClosestCreatureWithEntry(m_creature, NPC_VIMGOL_THE_VILE, 40.0f))
				{
			...
			{
				if (playersInsideCircles() == 5)
					m_creature->SummonCreature(NPC_VIMGOL_THE_VILE, m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ(), m_creature->GetOrientation(), TEMPSUMMON_TIMED_COMBAT_OR_DEAD_DESPAWN, 90000);
				if (m_uiBeamTimer == 0)
				{
					CastBunnySpell(nullptr, SPELL_PENTAGRAM_BEAM);
					m_uiBeamTimer = 5;
