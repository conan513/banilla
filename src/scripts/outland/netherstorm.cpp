...
struct npc_salhadaarAI : public ScriptedAI
{
	...
	void Reset() override
	{
		...
		m_creature->CastSpell(m_creature, SPELL_FLOAT, false);
	...
	void JustSummoned(Creature* pSummoned) override
	{
		...
		pSummoned->CastSpell(pSummoned, SPELL_SALAADIN_OVERSPARK, true);
	...
	void UpdateAI(const uint32 uiDiff) override
	{
		...
		if (m_uiAttackTimer)
		{
			if (m_uiAttackTimer <= uiDiff) // on attack start cast spell that handles everything
			{
				...
				if (!playerList.empty())
				{
					...
					m_creature->CastSpell(m_creature, SPELL_SALAADIN_OVERSPARK, true);
		...
		if (m_creature->isInCombat())
		{
			...
			{
				m_creature->CastSpell(m_creature, SPELL_MIRROR_IMAGE_1, true);
				m_creature->CastSpell(m_creature, SPELL_MIRROR_IMAGE_2, true);
				m_uiUsedMirrorImage = true;
			}
			else
			{
				...
				{
					m_uiGravityFlux = 15000;
					m_creature->CastSpell(m_creature, SPELL_GRAVITY_FLUX, false); // cast gravity flux every 15 seconds
					return;
				}
				else
				...
				if (m_uiStasisTimer <= uiDiff)
				{
					...
					{
						m_creature->CastSpell(target, SPELL_STASIS, false);  // cast stasis on random target every 22 seconds
...
struct npc_energy_ballAI : public ScriptedAI
{
	...
	void ReceiveAIEvent(AIEventType eventType, Creature* /*pSender*/, Unit* /*pInvoker*/, uint32 /*uiMiscValue*/) override
	{
		if (eventType == AI_EVENT_CUSTOM_A)
			m_creature->CastSpell(nullptr, SPELL_SALAADIN_TESLA, false);
	}
		...
		if (m_uiCastTimer)
		{
			if (m_uiCastTimer <= uiDiff)
			{
				...
				for (std::list<Creature*>::const_iterator itr = creatureList.begin(); itr != creatureList.end(); ++itr)
				{
					if (!(*itr)->isInCombat())
					{
						m_creature->CastSpell(m_creature, SPELL_SALAADIN_TESLA, false);
					}
				}
			}
			else
