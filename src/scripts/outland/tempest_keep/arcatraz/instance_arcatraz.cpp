...
void instance_arcatraz::Update(uint32 uiDiff)
{
    ...
    if (m_uiEntranceEventTimer)
    {
        if (m_uiEntranceEventTimer <= uiDiff)
        {
            ...
            if (Creature* pTemp = pPlayer->SummonCreature(uiEntry, aEntranceSpawnLoc[0], aEntranceSpawnLoc[1], aEntranceSpawnLoc[2], aEntranceSpawnLoc[3], TEMPSUMMON_TIMED_COMBAT_OR_DEAD_DESPAWN, 30000))
            {
                pTemp->SetWalk(false);
