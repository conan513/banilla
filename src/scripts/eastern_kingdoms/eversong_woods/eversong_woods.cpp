    ...
    void DoSpawnChallengers()
    {
        for (uint8 i = 0; i < MAX_CHALLENGER; ++i)
        {
            if (Creature* pCreature = m_creature->SummonCreature(uiChallengerId[i],
                                      ...
                                      TEMPSUMMON_TIMED_COMBAT_OR_DEAD_DESPAWN, 600000))
            {
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_uiWaveTimer)
        {
            if (m_uiWaveTimer <= uiDiff)
            {
                ...
                {
                    for (uint8 i = 0; i < 3; ++i)
                        m_creature->SummonCreature(NPC_ENRAGED_WRAITH, aSummonPos[i][0], aSummonPos[i][1], aSummonPos[i][2], aSummonPos[i][3], TEMPSUMMON_TIMED_COMBAT_OR_DEAD_DESPAWN, 5 * MINUTE);
                    m_uiWaveTimer = 29000;
                    m_bFirstWave = false;
                }
                else
